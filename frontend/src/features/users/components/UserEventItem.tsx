...
import { useKickUserFromEvent } from "../../events/hooks/useKickUserFromEvent";
...
export default function UserEventItem({ event, userId }: Props) {
  const router = useRouter();
  const { mutateKickUser } = useKickUserFromEvent();
  const items: PopoverItem[] = [
    ...
    },
    {
      ...
      destructive: true,
      onSelect: () => {
        mutateKickUser({ eventId: event.id, userId: userId });
      },
    },
  ];
